...
export default function Header({ data }: CommonType) {
  ...
  const btnChangeLang = Object.keys(data.languageNames[0]).map(
    (item, index) => (
      <ChangeLangButton key={index} lang={item} setIsMenuOpen={setIsMenuOpen} />
    ),
  )
  ...
  return (
    <>
      ...
      <header className="h-header z-10 w-[84%] mt-12 absolute left-[8%] sm:left-[12.5%] sm:w-[75%] sm:mt-4 ">
        <div
          className="w-full grid justify-between grid-cols-auto-1fr gap-x-6 px-3 bg-light 
        ...
        >
          ...
          <div className="w-full flex justify-end sm:hidden">
            <div
              className="w-11 h-11 bg-[#E6E4FD] rounded-full flex items-center justify-center"

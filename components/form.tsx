...
export default function FormContant({ data }: CommonType) {
  ...
  const SignupSchema = Yup.object().shape({
    ...
    phone: Yup.string()
      .test(
        'isVassslidNumber',
        data.contactUsFormErrorPhoneMissing,
        (phone?: string | any) => {
      ...
      .test(
        'isValidNumber',
        data.contactUsFormErrorPhoneInvalid,
        (phone?: string | null) => {
          const parsedNumber =
  ...
  const classError =
    'w-full h-8 text-2xl md:text-xl lg:text-base text-red font-normal'
  ...
  return (
    <WrapperLarge>
      ...
      >
          ...
          {({ errors, touched }) => (
            <Form>
              ...
              <div className={`${classDivInput} w-full`}>
                ...
                <Field
                  name="message"
                  className="w-full h-36 border-2 border-contact-bg rounded-lg"
                  component="textarea"
                />
                <div className={classError}>
              ...
              <button
                type="submit"
                className="font-bold text-2xl mt-9 w-44 h-14 bg-header-blue rounded-3xl 
              flex justify-center items-center text-light my-auto"
              >
        ...
        {modalSucsses && (
          <div
            className="absolute z-10 w-full bg-light/[.7] h-[44rem] bottom-0 left-0 
        border-2 border-contact-bg rounded-3xl text-2xl font-bold flex flex-col justify-center items-center"
          >
            {reactStringReplace(
              ...
              data.contactUsFormSuccessMessage.split('!')[1],
              (match, i) => (

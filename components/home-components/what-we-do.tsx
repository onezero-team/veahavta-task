    ...
    const getWhatWeDoCards = () => {
        return data.homepage?.whatWeDoCards?.map(
            (
                ...
                return (
                    <div key={index}>
                        <ServiceCard imageUrl={card.imagePath} title={card.title} description={card.text}/>
                    </div>
                )
            ...
            <div id={"what"} className={'h-[1450px] lgx:h-[1000px] mt-14'}>
                <div className={'w-full flex justify-center'}>
                    <div className={'flex flex-col items-center'}>
                        <div className={'cursor-default font-normal text-3xl mt-12 text-red'}>
                            {data.homepage.whatWeDoHeading}
                        </div>
                        <div className={'cursor-default hidden xml:block font-bold text-[82px]'}>
                ...
                <div className={'w-full flex justify-center card-main-container'}>
                    <div className={"w-[90vw] flex xml:w-auto relative"}>
                        <div ref={cardContainerRef} className={'flex card-container scrollbar-hide py-2 justify-between overflow-auto  xml:overflow-hidden xml:w-auto overflow-y-hidden space-x-5 rtl:space-x-reverse px-3 mt-14'}>
                            {getWhatWeDoCards()}
                        </div>
                        <div className={'absolute justify-between mt-64 xml:hidden w-full flex px-2'}>
                            <img
                                onClick={() => {
                                    // @ts-ignore
                                    cardContainerRef?.current?.scrollLeft +=
                                        scrollDirectionForAddXAxis('left')
                                }} className={'cursor-pointer -mx-5'} src={leftOrRight('left')}/>
                            ...
                            <img
                                ...
                                }} className={'cursor-pointer -mx-5'} src={leftOrRight('right')}/>
                        </div>
                    </div>
                </div>
            </div>

  ...
  return (
    <div className="ml-4 shadow-4xl rounded-[31px] w-[340px] h-[480px] overflow-hidden">
      ...
      <div className='p-5 pt-[30px]'>
        <h2 className={`text-accent-purple font-bold ${dir==='ltr'?'text-left':'text-right'}`}>{data.title}</h2>
        <p className={`text-paragraph leading-snug ${dir==='ltr'?'text-left':'text-right'}`}>{data.text}</p>
      </div>
    </div>
  )
}